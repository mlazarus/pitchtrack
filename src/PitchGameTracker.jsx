...
export default function PitchGameTracker() {
  ...
  const [stakes, setStakes] = useState({ gameScore: 4, bump: 2, points: 1, bonus: 25 });
  ...
  const confirmScore = async () => {
    ...
    if (editingHandIndex !== null) {
      ...
      setHands([...hands, { dealer, scoreA: sA, scoreB: sB }]);
  ...
  const saveStakes = async () => {
    ...
    if (existing) {
      await supabase.from('stakes').update(ns).eq('id', existing.id);
    } else {
      await supabase.from('stakes').insert(ns);
    }
  ...
  return (
    <div style={{ fontFamily: "'Manrope', 'Segoe UI', system-ui, sans-serif", background: '#0a0f1e', color: '#e8edf5', minHeight: '100vh' }}>
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)', padding: '1.5rem 2rem', borderBottom: '3px solid #4f46e5' }}>
        <h1 style={{ margin: 0, fontSize: '2rem', fontWeight: '800', letterSpacing: '-0.025em' }}>🎮 Pitch Game Tracker</h1>
        <div style={{ marginTop: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <label style={{ fontWeight: '600', fontSize: '0.9rem' }}>Table:</label>
          <select
            value={currentTable}
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: '0.5rem', padding: '1rem 2rem', borderBottom: '2px solid #1e293b', background: '#0f172a' }}>
        {['game', 'players', 'stats', 'settings', 'about'].map(tab => (
          <button
            key={tab}
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
        {activeTab === 'game' && (
          <div style={{ background: '#1e293b', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
            <h2 style={{ margin: '0 0 1rem 0', fontSize: '1.5rem', fontWeight: '700' }}>
              Game #<span style={{ color: '#6366f1' }}>{gameNumber}</span>
            </h2>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1.5rem' }}>
              ...
              <button onClick={resetDealers} style={buttonStyle('#f59e0b')}>Reset Dealers</button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div style={{ background: '#0f172a', padding: '1.5rem', borderRadius: '12px', border: '3px solid #10b981' }}>
                <h4 style={{ margin: '0 0 0.5rem 0', color: '#10b981', fontSize: '1rem' }}>Team A</h4>
                <div style={{ fontSize: '3rem', fontWeight: '800', color: '#10b981' }}>{totA.toFixed(0)}</div>
                <div style={{ marginTop: '0.5rem', color: '#94a3b8' }}>{teamA.join(', ') || 'No players'}</div>
              </div>
              <div style={{ background: '#0f172a', padding: '1.5rem', borderRadius: '12px', border: '3px solid #f59e0b' }}>
                <h4 style={{ margin: '0 0 0.5rem 0', color: '#f59e0b', fontSize: '1rem' }}>Team B</h4>
                <div style={{ fontSize: '3rem', fontWeight: '800', color: '#f59e0b' }}>{totB.toFixed(0)}</div>
                <div style={{ marginTop: '0.5rem', color: '#94a3b8' }}>{teamB.join(', ') || 'No players'}</div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '2rem', marginTop: '2rem' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', background: '#0f172a', borderRadius: '8px', overflow: 'hidden' }}>
                  <thead>
                    <tr style={{ background: '#334155' }}>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Dealer</th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>A</th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>B</th>
                    </tr>
                  </thead>
                  <tbody>
                    {hands.length === 0 ? (
                      ...
                      hands.map((h, idx) => {
                        ...
                        return (
                          ...
                          >
                            <td style={{ padding: '0.5rem', fontSize: '0.9rem' }}>{h.dealer}</td>
                            <td style={{ padding: '0.5rem', fontSize: '0.9rem', color: '#10b981', fontWeight: '600' }}>{rA.toFixed(1)}</td>
                            <td style={{ padding: '0.5rem', fontSize: '0.9rem', color: '#f59e0b', fontWeight: '600' }}>{rB.toFixed(1)}</td>
                          </tr>
                        );
                      })
                    )}
              ...
              <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', background: '#0f172a', borderRadius: '8px', overflow: 'hidden' }}>
                  <thead>
                    <tr style={{ background: '#334155' }}>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Team</th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Players</th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Game 1</th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Game 2</th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Game 3</th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr style={{ background: '#1e293b' }}>
                      <td style={{ padding: '0.75rem', fontWeight: '600' }}>A</td>
                      <td style={{ padding: '0.75rem' }}>{teamA.join(', ') || '-'}</td>
                      <td style={{ padding: '0.75rem' }}>{currentSetGames[0]?.scoreA.toFixed(0) || '-'}</td>
                      <td style={{ padding: '0.75rem' }}>{currentSetGames[1]?.scoreA.toFixed(0) || '-'}</td>
                      <td style={{ padding: '0.75rem' }}>{currentSetGames[2]?.scoreA.toFixed(0) || '-'}</td>
                      <td style={{ padding: '0.75rem', fontWeight: '700' }}>
                        {currentSetGames.length > 0 ? currentSetGames.reduce((sum, g) => sum + g.scoreA, 0).toFixed(1) : '-'}
                      </td>
                    </tr>
                    <tr style={{ background: '#0f172a' }}>
                      <td style={{ padding: '0.75rem', fontWeight: '600' }}>B</td>
                      <td style={{ padding: '0.75rem' }}>{teamB.join(', ') || '-'}</td>
                      <td style={{ padding: '0.75rem' }}>{currentSetGames[0]?.scoreB.toFixed(0) || '-'}</td>
                      <td style={{ padding: '0.75rem' }}>{currentSetGames[1]?.scoreB.toFixed(0) || '-'}</td>
                      <td style={{ padding: '0.75rem' }}>{currentSetGames[2]?.scoreB.toFixed(0) || '-'}</td>
                      <td style={{ padding: '0.75rem', fontWeight: '700' }}>
        ...
        {activeTab === 'stats' && (
          <div style={{ background: '#1e293b', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
            <h2 style={{ margin: '0 0 1.5rem 0', fontSize: '1.5rem', fontWeight: '700' }}>Statistics</h2>
            <p style={{ marginBottom: '0.5rem' }}>Games: <span style={{ color: '#6366f1', fontWeight: '700' }}>{completedGames.length}</span></p>
            <p>Hands: <span style={{ color: '#6366f1', fontWeight: '700' }}>{completedGames.reduce((sum, g) => sum + (g.handsPlayed || 0), 0)}</span></p>

            <h3 style={{ marginTop: '2rem', marginBottom: '1rem', fontSize: '1.25rem', fontWeight: '700' }}>Set History</h3>
            <table style={{ width: '100%', borderCollapse: 'collapse', background: '#0f172a', borderRadius: '8px', overflow: 'hidden' }}>
              <thead>
                <tr style={{ background: '#334155' }}>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Set</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Team A</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Team B</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>A Score</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>B Score</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: '700', fontSize: '0.85rem' }}>Winner</th>
                </tr>
              </thead>
              <tbody>
                {setHistory.length === 0 ? (
                  <tr>
                    <td colSpan={6} style={{ padding: '1rem', textAlign: 'center', color: '#64748b' }}>No sets yet</td>
                  </tr>
                ) : (
                  setHistory.map((s, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? '#1e293b' : '#0f172a' }}>
                      <td style={{ padding: '0.75rem' }}>{i + 1}</td>
                      <td style={{ padding: '0.75rem' }}>{s.teamA.join(', ')}</td>
                      <td style={{ padding: '0.75rem' }}>{s.teamB.join(', ')}</td>
                      <td style={{ padding: '0.75rem', color: '#10b981', fontWeight: '600' }}>{s.teamAScore.toFixed(1)}</td>
                      <td style={{ padding: '0.75rem', color: '#f59e0b', fontWeight: '600' }}>{s.teamBScore.toFixed(1)}</td>
                      <td style={{ padding: '0.75rem', fontWeight: '700' }}>{s.teamAScore > s.teamBScore ? 'Team A' : 'Team B'}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        {activeTab === 'settings' && (
          <div style={{ background: '#1e293b', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ display: 'grid', gap: '1rem', maxWidth: '400px' }}>
              ...
              <button onClick={saveStakes} style={buttonStyle('#6366f1')}>Save Stakes</button>
            </div>
          </div>
        )}
      ...
      <Modal show={showEndGameModal} onClose={() => setShowEndGameModal(false)}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <button onClick={() => endGameWithWinner('A')} style={{ ...buttonStyle('#10b981'), padding: '1rem' }}>Team A Wins</button>
          <button onClick={() => endGameWithWinner('B')} style={{ ...buttonStyle('#f59e0b'), padding: '1rem' }}>Team B Wins</button>
